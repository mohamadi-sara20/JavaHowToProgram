...
public class TestEmployeeCallingEarnings {

    public static double earning = 0.0;

    public static void main(String[] args) {

        BasePlusCommission basePlusCommission = new BasePlusCommission("Sahar", "Amadeh", "3456", 700, 0.1, 400);
        Hourly hourly = new Hourly("Saman", "Moh", "2345679", 10, 50);
        Salaried salaried = new Salaried("Mat", "Sep", "23456789", 1200);
        Commission commission = new Commission("Ali", "Samavati", "765", 900, 0.15);

        EmployeeImplementsPayable[] employeeImplementsPayable = new EmployeeImplementsPayable[4];

        employeeImplementsPayable[0] = basePlusCommission;
        employeeImplementsPayable[1] = hourly;
        employeeImplementsPayable[2] = salaried;
        employeeImplementsPayable[3] = commission;



        for(EmployeeImplementsPayable employeeImplementsPayable1: employeeImplementsPayable){
            System.out.print(employeeImplementsPayable1.toString());
            System.out.printf("%s: %.2f%n%n%n", "earned", employeeImplementsPayable1.getPaymentAmount());
